...
public class BlackJackController implements Initializable {
	...
	public void btnHit_Click(ActionEvent event) {
		...
		ScaleTransition scaleT2 = CreateScaleTransition1(img);
		
		//	Create a Path transition
		PathTransition pathT = CreatePathTransition(pntDeck, pntCardDealt, img);
		PathTransition pathT2 = CreatePathTransition1(pntDeck, pntCardDealt, img);

		//	Create a new Parallel transition.
		ParallelTransition patTMoveRot = new ParallelTransition();
		ParallelTransition patTshrink = new ParallelTransition();
		patTshrink.getChildren().addAll(scaleT2, pathT2);
		...
		seqDeal.getChildren().addAll(patTMoveRot, patTshrink, patTFadeInFadeOut);
...
//				toPoint.getY() / 3, toPoint.getX(), toPoint.getY()));
		...
		path.getElements().add( new QuadCurveTo(250, 250, toPoint.getX()/2, toPoint.getY()/2));
	...
	private PathTransition CreatePathTransition1(Point2D fromPoint, Point2D toPoint, ImageView img) {
		Path path = new Path();
		
		
		
		path.getElements().add(new MoveTo(toPoint.getX()/2, toPoint.getY()/2));
//		path.getElements().add(new CubicCurveTo(toPoint.getX() * 2, toPoint.getY() * 2, toPoint.getX() / 3,
//				toPoint.getY() / 3, toPoint.getX(), toPoint.getY()));
		//path.getElements().add(new CubicCurveTo(0, 120, 0, 240, 380, 240));
		path.getElements().add( new QuadCurveTo(250, 250, toPoint.getX(), toPoint.getY()));
		PathTransition pathTransition = new PathTransition();
		pathTransition.setDuration(Duration.millis(1000));
		pathTransition.setPath(path);
		pathTransition.setNode(img);
	...
	private ScaleTransition CreateScaleTransition1(ImageView img) {
		ScaleTransition st = new ScaleTransition(Duration.millis(iAnimationLength*(2)), img);
		st.setByX(-1.25f);
		st.setByY(-1.25f);
		st.setCycleCount((int) 1f);
